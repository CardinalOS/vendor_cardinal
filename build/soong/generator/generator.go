...
func init() {
	android.RegisterModuleType("cardinal_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cardinalExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cardinalExpandVariables(ctx, dep_file)
	...
	cmd := cardinalExpandVariables(ctx, String(g.properties.Cmd))
